...
for (j in 1:cancer_num) {
  print(j)
  #eval(parse(text=paste(cancer_dict[i,2],'_seg=read.delim("D:/my_study/Project/cancer_risk/data_git/TCGA_cnv/',cancer_dict[i,2],'.txt")',sep='')))
  #eval(parse(text=paste(cancer_dict[i,2],'_clinical=read.delim("D:/my_study/Project/cancer_risk/data_git/TCGA_cnv/',cancer_dict[i,2],'.txt")',sep='')))                      
  eval(parse(text=paste('tcga=read.delim("D:/my_study/Project/cancer_risk/data_git/TCGA_cnv/',cancer_dict[j,2],'.txt")',sep='')))
  eval(parse(text=paste('clinical=read.delim("D:/my_study/Project/cancer_risk/data_git/TCGA_cnv/',cancer_dict[j,2],'.tsv")',sep='')))   
  #filter 
  tcga_filtered=tcga[which(tcga[,6]< threshold_low | tcga[,6]> threshold_up),]
  #Store your own data as a GenomicRanges object
  #colnames(a) = c("Barcode", "chr", "start", "end", "extra1", "extra2")
  tcga_filtered_GR = makeGRangesFromDataFrame(tcga_filtered, keep.extra.columns = TRUE)
  #Overlap your regions with the reference dataset that you created
  hits = findOverlaps(genes_GR, tcga_filtered_GR, type="within")
  result = cbind(tcga_filtered[subjectHits(hits),],genes[queryHits(hits),])
  #合并clinical
  result_simple=result[,c(1,7,6)]
  result_simple[,1]=substr(result_simple[,1],1,12)
  clinical_simple=clinical[,c(2,4,6)]
  colnames(clinical_simple)[1]=colnames(result_simple)[1]
  join=full_join(result_simple,clinical_simple,by=colnames(clinical_simple)[1])
  #保留白人
  join=join[which(join$race=="white" ),]
  #删除赋值0.扩增赋值1
  ...
  female_num=length(which(clinical$gender=="female" & clinical$race=="white"))
  male_num=length(which(clinical$gender=="male" & clinical$race=="white"))
  
  ####不合并del和amp
  female_count=count(female[,c(2,3)])
  male_count=count(male[,c(2,3)])
  ...
  ####合并del和amp
  female_count_bind=count(female[,2])
  male_count_bind=count(male[,2])
  ...
  colnames(white_count_bind)=c("gene","female_1","female_0","male_1","male_0")
